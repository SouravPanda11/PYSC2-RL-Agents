...
class Agent(base_agent.BaseAgent):
    ...
    def initialize_first_step(self, obs):
        """
        Initialize variables during the first observation of the episode.
        """
    ...
    def get_hot_squares(self, obs):
        """
        Calculate hot squares on the minimap where enemy units are present.
        """
    ...
    def get_current_state(self, obs):
        """
        Extract and return the current state of the environment.
        """
        unit_type = obs.observation['feature_screen'][_UNIT_TYPE]
        ...
        # Get the hot squares and update the state vector
    ...
    def handle_first_step(self, obs, smart_action, x, y, unit_type):
        """
        Handle the first step of the agent's decision-making process.
        """
        if smart_action in [ACTION_BUILD_BARRACKS, ACTION_BUILD_SUPPLY_DEPOT]:
    ...
    def handle_second_step(self, obs, smart_action, x, y, unit_type):
        """
        Handle the second step of the agent's decision-making process.
        """
    ...
    def handle_third_step(self, obs, smart_action, x, y, unit_type):
        """
        Handle the third step of the agent's decision-making process.
        """
        if smart_action in [ACTION_BUILD_BARRACKS, ACTION_BUILD_SUPPLY_DEPOT]:
...
# Sparse Reward Agent class
class SparseRewardAgent(Agent):
    def __init__(self):
        super(SparseRewardAgent, self).__init__()
        self.qlearn = QLearningTable(actions=list(range(len(smart_actions))))
        self.previous_action = None
        self.previous_state = None

    def handle_last_observation(self, obs):
        """
        Handle the last observation in the episode.
        """
        reward = obs.reward
    ...
    def step(self, obs):
        super(SparseRewardAgent, self).step(obs)

        # Handle the last observation of the episode
        if obs.last():
            return self.handle_last_observation(obs)

        # Handle the first observation of the episode
        if obs.first():
            self.initialize_first_step(obs)
        ...
        if self.move_number == 0:
            ...
            smart_action, x, y = self.split_action(rl_action)
            return self.handle_first_step(obs, smart_action, x, y, obs.observation['feature_screen'][_UNIT_TYPE])

        elif self.move_number == 1:
            self.move_number += 1
            smart_action, x, y = self.split_action(self.previous_action)  # Unpack here
            return self.handle_second_step(obs, smart_action, x, y, obs.observation['feature_screen'][_UNIT_TYPE])

        elif self.move_number == 2:
            self.move_number = 0
            smart_action, x, y = self.split_action(self.previous_action)  # Unpack here
            return self.handle_third_step(obs, smart_action, x, y, obs.observation['feature_screen'][_UNIT_TYPE])
