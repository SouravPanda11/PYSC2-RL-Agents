...
class QLearningTable: 
  ...
  # Check to see if the state is in the QTable already, and if not it will add it with a value of 0 for all possible actions.
  def check_state_exist(self, state):
    if state not in self.q_table.index:
        ...
        # Append the series to the dataframe using concat
        self.q_table = pd.concat([self.q_table, pd.DataFrame(new_series).T])
...
def main(unused_argv):
  ...
  try:
    with sc2_env.SC2Env(
      ...
      run_loop.run_loop([agent1, agent2], env, max_episodes=10) # Control both agents instead of one
